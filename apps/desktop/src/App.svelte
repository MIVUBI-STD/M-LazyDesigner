    ...
    <section id="main-content" class="main-view" tabindex="-1">
      ...
      {:else if page === 'Overview'}
        ...
        <main class="content centered-content">
          ...
          {/if}
          {#if (productMode(status) === 'welcome' && !status.bootstrap_available) || (productMode(status) === 'security-setup' && !status.maintenance.setup_tls)}
            <section class="guidance-card"><strong>Setup unavailable</strong><p>{productGuidance(status)}</p></section>
          {/if}
...
<style>
  ...
  .more-menu{position:relative}.more-menu summary{width:38px;height:38px;display:grid;place-items:center;list-style:none;border-radius:8px;color:var(--muted);cursor:pointer}.more-menu summary:hover{background:var(--surface-2);color:var(--text)}.more-menu[open] summary{background:var(--surface-2);color:var(--text)}.more-menu summary::-webkit-details-marker{display:none}.menu-popover{position:absolute;z-index:20;right:0;top:42px;width:180px;display:grid;padding:6px;animation:menu-in 110ms ease-out;border:1px solid var(--border);border-radius:10px;background:var(--surface-2);box-shadow:var(--shadow-popover)}.menu-popover button{width:100%;padding:8px 9px;border-radius:7px;background:transparent;color:var(--text-soft);text-align:left;cursor:pointer;font-size:10px}.menu-popover button:hover:not(:disabled){background:var(--surface-3)}
  ...
  @keyframes attention-in{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}}@keyframes menu-in{from{opacity:0;transform:translateY(-3px)}to{opacity:1;transform:translateY(0)}}
