...
function validateDimensions(width: number, height: number): void {
  if (!Number.isInteger(width) || width <= 0 || !Number.isInteger(height) || height <= 0) {
    throw new Error("Geometry signal dimensions must be positive integers.");
  }
}
...
function rectMask(width: number, height: number, regions: readonly TextureRect[]): ScalarMask {
  ...
  for (const rect of regions) {
    if (![rect.x, rect.y, rect.width, rect.height].every(Number.isFinite) || rect.width <= 0 || rect.height <= 0) {
      throw new Error("Geometry signal region must contain finite positive bounds.");
    }
    ...
    for (let y = minY; y < maxY; y += 1) {
      for (let x = minX; x < maxX; x += 1) values[y * width + x] = 1;
    }
  }
...
function uniformMask(width: number, height: number, value: number): ScalarMask {
  const values = new Float32Array(width * height);
  values.fill(Math.max(0, Math.min(1, value)));
  return { width, height, values };
}
...
function lowerMask(width: number, height: number, power: number): ScalarMask {
  ...
  for (let y = 0; y < height; y += 1) {
    ...
    const value = Math.pow(t, power);
    for (let x = 0; x < width; x += 1) values[y * width + x] = value;
  }
  return { width, height, values };
}
...
export function deriveGeometrySurfaceSignals(evidence: GeometrySignalEvidence) {
  ...
  if (evidence.edge_falloff !== undefined) {
    result.edge = edgeMask(evidence.width, evidence.height, evidence.edge_falloff);
  }
  if (evidence.cavity_regions?.length) {
    result.cavity = rectMask(evidence.width, evidence.height, evidence.cavity_regions);
  }
  if (evidence.contact_regions?.length) {
    result.contact = rectMask(evidence.width, evidence.height, evidence.contact_regions);
  }
  if (evidence.world_normal) {
  ...
  if (evidence.lower_grime_power !== undefined) {
    result.lower_exposure = lowerMask(evidence.width, evidence.height, evidence.lower_grime_power);
  }
  return result;
}
