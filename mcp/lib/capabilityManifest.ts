import { classifyMcpToolPhaseByName } from "./authoringPhase";
...
export type CapabilityAuthoringDomain = "GEOMETRY" | "TEXTURING" | "ANIMATION" | "CORE";
...
export type CapabilityManifestEntry = CapabilityMetadata & {
  authoringDomain: CapabilityAuthoringDomain;
};

export function getCapabilityManifestEntry(name: string): CapabilityManifestEntry {
  const phase = classifyMcpToolPhaseByName(name);
  const authoringDomain: CapabilityAuthoringDomain =
    phase === "geometry"
      ? "GEOMETRY"
      : phase === "texturing"
        ? "TEXTURING"
