...
const GATEWAY_INSTRUCTIONS =
  "Stable LazyDesigner Gateway boundary with LazyDesigner Control. Start/resume with status only when orientation is unknown or materially stale; reuse the compact Control packet and content-addressed context handles. For asset work, pass reference_package_path when a LazyDesigner Reference Package exists; Control projects only the active authoring stage plus exactly one selected modelling profile for Geometry. For product/source work, use task_mode=SYSTEM_DEVELOPMENT with concrete task_intent so Control returns bounded source/specialist/test ownership instead of broad repository scans. Known Runtime capability → invoke directly; search only when unknown/stale and describe only for real schema uncertainty. Reuse control_delta.verification_class: receipt_only means no confirmation read, focused_read means inspect only when the returned state is insufficient, and visual means refresh only evidence needed for the next visual decision. Geometry/Texturing share AUTHORING; Animation is the only Runtime phase handoff. invoke_capability never auto-retries an interrupted mutation.";
